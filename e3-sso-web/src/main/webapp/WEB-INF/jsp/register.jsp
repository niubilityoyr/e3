...
<div class="tabBox clear">
    ...
    <span class="reg_hide" id="regcom" style="">企业用户</span>    <div class="login">已有账号，立即 <a href="/page/login.html">登录</a></div>
</div>
<!--个人用户-->
...
<script type="text/javascript">
	var REGISTER={
		...
		beforeSubmit:function() {
				//检查用户是否已经被占用
				$.ajax({
	            	url : REGISTER.param.surl + "/user/check/"+escape($("#regName").val())+"/1.do?r=" + Math.random(),
	            	success : function(data) {
	            		if (data.data) {
	            			//检查手机号是否存在
	            			$.ajax({
	            				url : REGISTER.param.surl + "/user/check/"+$("#phone").val()+"/2.do?r=" + Math.random(),
				            	success : function(data) {
				            		if (data.data) {
					            		REGISTER.doSubmit();
				            		} else {
		...
		doSubmit:function() {
			$.post("/user/register.do", $("#regForm_mod").serialize(), function(data){
				if(data.status == 200){
